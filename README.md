...
- **`split-claude-md`** — four-phase `CLAUDE.md` maintenance. Phase A rotates old dated decision-log entries into monthly archive files and builds an index with a grep-pattern navigation header. Phase B consolidates recurring failure patterns (two or more instances) into a single "Known Patterns & Gotchas" section with instance counts. Phase C extracts stable reference sections (architecture, file maps, data layouts) into standalone files (`ARCHITECTURE.md`, `DEVELOPMENT.md`, and so on) with one-line pointer stubs left in `CLAUDE.md`. Phase D is for the sections Phase C must never touch: a rule moved out of an always-loaded file stops being read, so the file gets smaller while the behaviour it governed quietly stops happening. Phase D cuts within such a section instead of between sections, leaving the rule and moving only its evidence. The goal is to keep the always-loaded `CLAUDE.md` lean. Each phase is skippable; the default runs all four.
